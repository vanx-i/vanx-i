...
import { sendGameReleaseEmail } from '@/lib/email'
...
export async function GET() {
  try {
    ...
    // Buscar juegos que salen hoy y usuarios interesados no notificados
    ...
    if (releases.length === 0) {
      return NextResponse.json({ message: 'No hay lanzamientos hoy' })
    }
    ...
    for (const release of releases) {
      try {
        ...
        await sql`
          UPDATE user_games SET notified = true WHERE id = ${release.user_game_id}
        `
        notified++
      } catch (emailError) {
