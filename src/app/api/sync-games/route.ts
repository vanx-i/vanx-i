...
export async function GET() {
  try {
    ...
    if (!games || games.length === 0) {
      return NextResponse.json({ message: 'No se encontraron juegos' })
    }
    ...
    return NextResponse.json({ message: `${inserted} juegos sincronizados correctamente` })
  } catch (error) {
    if (error instanceof Error) console.error(error.message)
    return NextResponse.json({ error: 'Error sincronizando juegos' }, { status: 500 })
  }
}